...
class SetOutput():
    ...
    def __init__(self):
        self.data_path = os.getenv(const.CI_DATA_DIR, '.')
        self.output_file_name = os.getenv(const.CI_DATA_OUTPUT, 'output.json')
    ...
    def check_output(self, output):
        ...
        if not status or status not in const.CI_PLUGIN_STATUS.values():
            self._log.error("[check output error]invalid status:{}".format(status))
            exit(-1)

        output_template_type = output.get("type", None)
        if not output_template_type or output_template_type not in const.CI_OUTPUT_TEMPLATE_TYPE.values():
        ...
        for k, v in output_data.items():
            ...
            if field_type == const.CI_OUTPUT_FIELD_TYPE.get("STRING", ""):
                pass
            elif field_type == const.CI_OUTPUT_FIELD_TYPE.get("ARTIFACT", ""):
                ...
                for file_path in field_value:
                    if not os.path.exists(file_path):
                        ...
                        exit(-1)
            elif field_type == const.CI_OUTPUT_FIELD_TYPE.get("REPORT", ""):
                pass
            else:
    ...
    def set_output(self, output):
        ...
        if not os.path.exists(self.data_path):
            try:
                ...
                self._log.debug("mkdir data_path error: ", e)
                pass
